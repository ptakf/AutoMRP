...
export function calculateMrps() {
    calculateMrp(shoe, shoeInsert);
    calculateMrp(shoeInsert, cardboard);
    calculateMrp(shoeInsert, protectiveLayer);

    calculateMrp(shoe, shoeLace);

    calculateMrp(shoe, incompleteShoe);
    calculateMrp(incompleteShoe, shoeSole);
    calculateMrp(incompleteShoe, upperLayer);
}
var shoe = {
    BOMlevel: 0,
};
...
function calculateMrp(higerBOM, lowerBOM) {
    ...
    for (let i = 0; i < mpsCalculator.weekAmount; i++) {
        ...
        plannedOrderReleases.push(i > 0 ? (netDemand < 0 ? lotSize : 0) : 0);
    ...
    console.log("Planned orders:           ", plannedOrderReleases.join(", "));
    console.log("Planned receipt of orders:", plannedOrderReceipts.join(", "));
    console.log("------------------------");
